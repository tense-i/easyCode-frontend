    ...
    <el-tabs type="border-card">
      ...
      <el-tab-pane v-if="0 !== (showTab & 2)" label="生成表">
        ...
        >
          ...
          <el-form-item>
            <el-button type="primary" @click="importGenTableEmit"
              >导入</el-button
            >
          </el-form-item>
        </el-form>
      ...
      <el-tab-pane v-if="0 !== (showTab & 4)" label="数据源">
        <el-space :size="10" spacer="">
          ...
          <el-button type="primary" @click="importDynamicTableEmits"
            >导入
          </el-button>
        </el-space>
        <div style="margin: 20px 0"></div>
        <trends-table
          ref="importDynamicTable"
          :max-height="300"
...
function doPageGenTable() {
  if (localShared.value.value) {
    pageSharedGenTable(
      proxy.addDateRange(queryGenTableParams, dateRange.value)
    ).then((res) => {
      listGenTableData.value = res.data.records;
      queryGenTableParams.total = Number(res.data.total);
    });
  } else {
    pageGenTable(proxy.addDateRange(queryGenTableParams, dateRange.value)).then(
      (res) => {
        listGenTableData.value = res.data.records;
        queryGenTableParams.total = Number(res.data.total);
      }
