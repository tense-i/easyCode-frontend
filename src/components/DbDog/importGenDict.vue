<template>
  <!-- 导入词库 -->
    ...
    <el-drawer v-if="witch === 'drawer'"
        ...
        <el-table v-loading="loading" :data="queryPageData" @selection-change="handleSelectionChange">
    ...
    <el-dialog  v-if="witch === 'dialog'"
        title="导入词库" v-model="visible" width="800px" top="5vh" append-to-body draggable>
...
function getList() {
    ...
    if (!shared) {
        listMyAddDictByPage(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
            if (res.code === 200) {
        ...
        listSharedDictByPage(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
            if (res.code === 200) {
                data.queryPageData = res.data.records;
                data.queryParams.total = Number(res.data.total);
                loading.value = false;
