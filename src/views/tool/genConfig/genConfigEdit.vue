<template>
  <div class="app-container">
    <el-tabs v-model="activeName">
      <el-tab-pane label="基本信息" name="genInfo">
        <div>
          <el-form :model="propertiesInfo" label-width="150px">
            <el-row>
              <el-col :span="12">
              ...
              <el-col :span="12">
                <el-form-item prop="moduleName">
                  <template #label>
                    生成模块名
                    <el-tooltip
                      content="可理解为子系统名，例如 system vm变量名为 moduleName"
                      placement="top"
                    >
                      <el-icon>
                        <question-filled />
              ...
              <el-col :span="12">
                <el-form-item prop="tablePrefixArray">
                  <template #label>
                    表前缀
                    <el-tooltip
                      content="表前缀（可多个值，使用，分割）如 表名为 easycode_user_info,前缀为 easycode，那么生成className为userInfo"
                      placement="top"
                    >
                      <el-icon>
                        <question-filled />
              ...
              <el-col :span="12">
                <el-form-item prop="shared" label="是否共享">
                  <el-select
                    v-model="propertiesInfo.shared"
                    clearable
                    placeholder="Select"
                    style="width: 240px"
                  >
      ...
      <el-tab-pane label="VM映射信息" name="columnMapping">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-space size="10" spacer="|">
              <el-tooltip content="物理类型与Java字段映射关系" placement="top">
                <el-text>
                  字段映射
                  <el-icon>
                    <question-filled />
            ...
            <el-button
              class="mt-4"
              style="width: 100%"
              @click="handleAddOfColumnMapping(keyValueNew)"
            >
              Add Item
            </el-button>
          </el-col>
          ...
          <el-col :span="12">
            <el-space size="10" spacer="|">
              <el-tooltip content="自定义vm变量" placement="top">
                <el-text>
                  自定义映射
                  <el-icon>
                    <question-filled />
      ...
      <el-tab-pane label="生成模板" name="vm">
        <div style="margin-bottom: 20px">
          ...
          <el-button
            type="primary"
            @click="handleUpdateOrSaveVms([getActiveTab()])"
          >
            保存当前编辑
          </el-button>
          <el-button
          ...
          <el-button
            type="primary"
            style="float: right"
            @click="handleUpdateOrSaveVms(data.propertiesInfo.vms)"
          >
            保存全部文件
          </el-button>
          <el-button
            ...
            style="float: right"
            @click="
              handlePreviewCode(data.propertiesInfo.vms, data.propertiesInfo)
            "
          >
            预览全部文件
          </el-button>
              ...
              <el-row>
                ...
                <el-col :span="4" style="margin-right: 20px">
                  <el-form-item>
                    <template #label>
                      拓展名
                      <el-tooltip content="生成文件拓展名" placement="top">
                        <el-icon>
                          <question-filled />
                        </el-icon>
                      </el-tooltip>
...
const { propertiesInfo, preview } = toRefs(data);

watch(
  () => data.propertiesInfo.basicInformations,
  (newVal, oldVal) => {
    data.propertiesInfo.basicInformation = JSON.stringify(
      data.propertiesInfo.basicInformations
    );
  },
  { deep: true }
...
watch(
  () => data.propertiesInfo.columnMappingInformations,
  (newVal, oldVal) => {
    data.propertiesInfo.columnMappingInformation = JSON.stringify(
      data.propertiesInfo.columnMappingInformations
    );
  },
  { deep: true }
...
function validateProperties() {
  // 去除空格并检查 packagePath
  let trimmedpackagePath = data.propertiesInfo.packagePath
    ? data.propertiesInfo.packagePath.trim()
    : null;
  let packagePathIsValid =
    trimmedpackagePath !== null && trimmedpackagePath !== "";

  // 去除空格并检查 moduleName
  let trimmedModuleName = data.propertiesInfo.moduleName
    ? data.propertiesInfo.moduleName.trim()
    : null;
  let moduleNameIsValid =
    trimmedModuleName !== null && trimmedModuleName !== "";
