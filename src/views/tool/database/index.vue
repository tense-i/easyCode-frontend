          ...
          <el-dropdown>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  @click="
                    openDialogDataSourceModel(mysqlDynamicDatabaseDataDto)
                  "
                  >MySQL
                </el-dropdown-item>
                <el-dropdown-item
                  @click="openDialogDataSourceModel(pgDynamicDatabaseDataDto)"
                >
                  PostgreSQL
                </el-dropdown-item>
              </el-dropdown-menu>
...
import {
  databaseInfo,
  listDynamicColumn,
  listDynamicDatabaseData,
  listDynamicTable,
} from "@/api/system/database.js";
import {
  deleteDataSourceModel,
  updateOrSaveDataSourceModel,
} from "@/api/system/dataSourceModel.js";
...
let mysqlDataSourceModel = new DataSourceModel(
  null,
  null,
  "MySQL",
  "127.0.0.1",
...
let pgDataSourceModel = new DataSourceModel(
  null,
  null,
  "PostgreSQL",
  "127.0.0.1",
...
let mysqlDynamicDatabaseDataDto = new DynamicDatabaseDataDto(
  null,
  mysqlDataSourceModel
);
let pgDynamicDatabaseDataDto = new DynamicDatabaseDataDto(
  null,
  pgDataSourceModel
);
...
function doUpdateOrSaveDataSourceModel() {
  // debgger
  let save =
    typeof formDataSourceModel.value.id === "undefined" ||
    formDataSourceModel.value.id === null ||
    formDataSourceModel.value.id === "";
...
function doDdeleteDataSourceModel(row) {
  deleteDataSourceModel([row.value.id]).then((res) => {
    // debgger
    data.databaseDtos = data.databaseDtos.filter(
      (dto) => dto.value.id !== row.value.id
    );
...
function handleOpen(key, keyPath) {
  ...
  doListDynamicDatabaseData(
    database != null ? database : dataSourceModel,
    dataSourceModel.value,
    database != null ? database.name : null,
    query
  );
}
...
function doListDynamicDatabaseData(
  dynamicDatabaseDataDto,
  dataSourceModel,
  databaseName,
  query
...
function viewTable(dataSourceModel, queryDatabaseName, tableName) {
  ...
  let targetName =
    tableName + " @" + databaseName + "(" + connectionName + ") - 库";
  // 是否已经初始化的标志
  let isExist = false;
