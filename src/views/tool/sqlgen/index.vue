                    ...
                    >
                      <template #default="inscope">
                        <el-button
                          type="primary"
                          link
                          @click="handleSaveGenTableColumn(inscope.row)"
                          >Save
                        </el-button>
                        <el-button
                          type="danger"
          ...
          >
            <template #default="scope">
              <el-button
                type="primary"
                link
                @click="handleSaveGenTable(scope.row)"
                >Save</el-button
              >
              <el-button
                type="danger"
      ...
      <el-collapse-item name="2">
        ...
        >
          ...
          >
            <el-tabs v-model="generateCode.tabName2" type="border-card">
              <el-tab-pane
                v-for="(item2, key2) in generateCode.data[
                  generateCode.tabName1
                ]"
                :label="item2.fileName"
                :name="item2.vmId"
                :key="item2.vmId"
...
import {
  downloadCodeByGenConfigAndGenTables,
  previewCodeByGenConfigAndGenTables,
} from "@/api/system/codeGenerator.js";
...
function handleImportGenConfig(rows) {
  if (rows.length === 1) {
    ...
    proxy.$modal.msgSuccess(
      "选中第一个配置 [" + data.p.genConfig.name + "] 配置"
    );
  } else if (rows.length === 0) {
...
function doDownloadCodeByGenConfigAndGenTables() {
  ...
  proxy.downloadPlus(
    "/dbdog/generator/downloadCodeByGenConfigAndGenTables",
    queryData,
    `dbdog.zip`
  );
}
