<template>
    <div class="app-container">
        ...
        <trends-table :show-columns="showColumnsGenTable"
                      :columns="tableList"
                      :selection="true"
                      :index="true"
        ...
        <import-gen-table ref="importRef"
                          :show-tab="7"
                          @ok="handleQuery"
                          @import-sql="importSql"
                          @importDynamicTable="importDynamicTable"
...
import {listTable, previewTable, delTable, genCode, synchDb, updateGenTable} from "@/api/tool/gen";
...
function getList() {
    loading.value = true;
    pageGenTable(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => {
...
function handleEditTable(row) {
    ...
    router.push({path: "/tool/gentable/edit/index/" + tableId, query: {pageNum: queryParams.value.pageNum}});
}
